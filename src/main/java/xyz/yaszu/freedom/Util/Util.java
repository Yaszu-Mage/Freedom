...
public class Util {
    ...
    public void drawElipse(Location center, double radius, int points, Particle particle, double linearscale, double horizontalscale){
    ...
    public void drawEye(Location location,int scale) {
        drawElipse(location,1 + scale,128,Particle.DUST,-2.58,6.6);
        drawElipse(location,1+ scale,128,Particle.DUST,-2.58,3.3);
        drawCircle(location,2+ 2 * scale, location.getWorld(), 128,Particle.DUST);
    }
    ...
    public void createMinMagicCircleAroundPlayer(Player target, int tickRate) {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                drawCircle(target.getLocation(),1,target.getWorld(),30,Particle.DUST);
                tick = tick + tickRate;
                if (tick >= 1000) {
                    this.cancel();
                }
    ...
    public void drawSquare(Location center, double size, double rot, Particle particle, Particle.DustOptions options,double xlimit, double ylimit, double zlimit) {
